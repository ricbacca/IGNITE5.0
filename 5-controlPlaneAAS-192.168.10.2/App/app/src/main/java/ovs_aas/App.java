...
public class App {
    ...
    public static void main(String[] args) {
        ...
        IShell networkInfrastructure = new NetworkControlPlane(
    ...
    private static void waitForRegistry() {
        String registryAddress = StaticProperties.get_REGISTRY_IP().split(":")[0];
        int registryPort = Integer.parseInt(StaticProperties.get_REGISTRY_IP().split(":")[1]);

        System.out.println("Waiting for registry at: " + registryAddress + ":" + registryPort);

        while(!isReachable(registryAddress, registryPort, 20000)) 
            System.out.println("REGISTRY IP NOT REACHABLE");

        while(!client.isServerAvailable(StaticProperties.get_REGISTRY_PATH() +  StaticProperties.REGISTRY_POLLING)) {
        System.out.print("WAITING FOR REGISTRY API");
            try {
                Thread.sleep(1000);
    ...
    /*private static void waitForControllers() {
        System.out.print("Waiting for Controlelrs at " + StaticProperties.getCNT1_IP() + ", " + StaticProperties.getCNT2_IP());
    ...
    private static void parseAndStore() {
        ...
        String cnt1_ip = System.getenv("CNT1_IP");
        String cnt2_ip = System.getenv("CNT2_IP");
        String registry_ip = System.getenv("REGISTRY_IP");

        if (aas_ip == null || cnt1_ip == null || cnt2_ip == null || registry_ip == null) {
