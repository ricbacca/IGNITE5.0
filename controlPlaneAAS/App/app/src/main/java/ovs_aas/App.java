...
public class App {
    ...
    private static void waitForRegistry() {
        System.out.print("Waiting for Registry at " + StaticProperties.get_REGISTRY_IP().split(":")[0]);

        InetAddress registryAddress;
        try {
            registryAddress = InetAddress.getByName(StaticProperties.get_REGISTRY_IP().split(":")[0]);
            while(!registryAddress.isReachable(20000)) 
